...
class LlamaCppConfig:
    def __init__(self, name:str, binary: str|Path, model:str|Path, params: dict|None = None):
        self.name = name
    ...
    def with_params(self, **params: str|float|bool|Path) -> 'LlamaCppConfig':
        ...
        return LlamaCppConfig(name=self.name, binary=self.binary, model=self.model, params=new_params)
    ...
    def build(self) -> LlamaSwapModel:
        ...
        quoted_cmdline = shlex.quote(str(self.binary)) + " -m " + shlex.quote(str(self.model))
        for k, v in self._params.items():
            k = k.replace("_", "-")  # Replace underscores with hyphens for command line flags
            if v is None:
                quoted_cmdline += f" --{k}"
        ...
        logger.debug(f"Building LlamaSwapModel for {self.name} with command: {quoted_cmdline}")
